...
def self_blend(image_rgb: np.ndarray, params: SBIParams, rng: Optional[np.random.Generator] = None, *, landmarks: Optional[np.ndarray] = None) -> tuple[np.ndarray, bool, np.ndarray]:
    rng = rng or np.random.default_rng()
    if rng.random() >= params.p:
        return image_rgb, False, np.zeros(image_rgb.shape[:2], dtype=np.float32)

    h, w = image_rgb.shape[:2]
    landmarks = _try_landmarks(image_rgb) if landmarks is None else landmarks
