...
class CalibrationParams:
    ...
    def apply(self, logits: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        ...
        # ``fit_calibrator`` estimates prior_bias after temperature scaling:
        #     prior_bias = mean(logits / T)
        # Therefore the same scaled-logit convention must be used here.
        # The previous implementation evaluated (logits - prior_bias) / T,
        # which divided the already scaled bias by T a second time.
        z = logits.astype(np.float32) / T - float(self.prior_bias)
