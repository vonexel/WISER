...
class Trainer:
    def __init__(self, model: nn.Module, loss_fn: nn.Module, cfg, train_loader: DataLoader, val_loader: DataLoader,
        ...
        self.optimizer = build_optimizer(self.model, cfg.training.optim, self.loss_fn)
    ...
    def _train_one_epoch(self, epoch: int) -> dict:
        ...
        if self.cfg.training.sanity_steps > 0:
            steps = min(steps, self.cfg.training.sanity_steps)
        batches = progress(islice(self.train_loader, steps), total=steps,
                           desc=f"Train epoch {epoch + 1}/{self.cfg.training.epochs}")
        for step, batch in enumerate(batches):
            ...
            n += int(len(labels))
            batches.set_postfix(loss=f"{total / n:.4f}", refresh=False)
    ...
    def _validate(self, epoch: int) -> dict:
        ...
        n = 0
        total_loss = 0.0
        for batch in progress(self.val_loader, desc=f"Validation epoch {epoch + 1}"):
            rgb = batch["rgb"].to(self.device, non_blocking=True)
            wav = batch["wavelet"].to(self.device, non_blocking=True)
            df = batch["defocus"].to(self.device, non_blocking=True)
            ...
            with self.amp_ctx:
                ...
                if "method_label" in batch:
                    out["method_label"] = batch["method_label"].to(self.device, non_blocking=True)
        ...
        # Select the same EMA weights that will actually be evaluated.
        auc_for_stop = m.auc
    ...
    def fit(self) -> TrainerOutput:
        ...
        for epoch in range(self.cfg.training.epochs):
            ...
            if self.cfg.training.eval_during_training:
                ...
                history.append({"epoch": epoch + 1, "train_loss": train_loss_last,
                                "val_loss": val_loss_last, "val_auc_ema": float(vs["auc_ema"]),
                                "lr": lr_history[-1]})
                with open(self.out_dir / "epoch_history.json", "w", encoding="utf-8") as f:
